...
namespace gtsst::compressors {
    DecompressionConfiguration SlidingV1Compressor::configure_decompression(size_t buf_size) {
    ...
    GTSSTStatus SlidingV1Compressor::decompress(const uint8_t* src, uint8_t* dst, DecompressionConfiguration& config,
                                                size_t* out_size) {
        // Read file header
        ...
        for (int block_id = 0; block_id < file_header.num_blocks; block_id++) {
            fsst::DecodingTable decoder = decoders[block_id / slidingv1::SUPER_BLOCK_SIZE];
