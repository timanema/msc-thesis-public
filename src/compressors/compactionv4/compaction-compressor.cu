...
#include <compressors/compactionv4/compaction-defines.cuh>
#include <compressors/compactionv4/compaction-encode.cuh>
...
namespace gtsst::compressors {
    CompressionConfiguration CompactionV4Compressor::configure_compression(const size_t buf_size) {
        return CompressionConfiguration{
            ...
            .min_alignment_input = compactionv4::WORD_ALIGNMENT,
            .min_alignment_output = compactionv4::WORD_ALIGNMENT,
            .min_alignment_temp = compactionv4::TMP_WORD_ALIGNMENT,
            .must_pad_alignment = true,
            .block_size = compactionv4::BLOCK_SIZE,
            .table_range = compactionv4::BLOCK_SIZE * compactionv4::SUPER_BLOCK_SIZE,
            .must_pad_block = true,
    ...
    GTSSTStatus CompactionV4Compressor::validate_compression_buffers(const uint8_t* src, uint8_t* dst, uint8_t* tmp,
                                                                     CompressionConfiguration& config) {
        if (config.input_buffer_size > compactionv4::BLOCK_SIZE * (size_t)0xFFFFFFFF) {
            return gtsstErrorTooBig;
        }

        if (config.block_size != compactionv4::BLOCK_SIZE) {
            return gtsstErrorBadBlockSize;
        }

        if (config.min_alignment_input != compactionv4::WORD_ALIGNMENT ||
            config.min_alignment_output != compactionv4::WORD_ALIGNMENT ||
            config.min_alignment_temp != compactionv4::TMP_WORD_ALIGNMENT) {
            return gtsstErrorBadBlockSize;
        }

        if ((uintptr_t)src % compactionv4::WORD_ALIGNMENT != 0 || (uintptr_t)dst % compactionv4::WORD_ALIGNMENT != 0 ||
            (uintptr_t)tmp % compactionv4::TMP_WORD_ALIGNMENT != 0) {
            return gtsstErrorAlignment;
        }

        if (config.input_buffer_size % compactionv4::BLOCK_SIZE != 0 ||
            config.temp_buffer_size % compactionv4::TMP_OUT_BLOCK_SIZE != 0) {
            return gtsstErrorBlockAlignment;
        }

        if (config.input_buffer_size % compactionv4::WORD_ALIGNMENT != 0 ||
            config.temp_buffer_size % compactionv4::TMP_WORD_ALIGNMENT != 0) {
    ...
    GTSSTStatus CompactionV4Compressor::compress(const uint8_t* src, uint8_t* dst, const uint8_t* sample_src,
        ...
        // Some bookkeeping
        const uint64_t number_of_blocks = config.input_buffer_size / compactionv4::BLOCK_SIZE;
        const uint64_t working_block_size = number_of_blocks * compactionv4::TMP_OUT_BLOCK_SIZE;
        const uint64_t number_of_tables = (number_of_blocks - 1) / compactionv4::SUPER_BLOCK_SIZE + 1;
        const uint64_t metadata_mem_size = sizeof(compactionv4::GCompactionMetadata) * number_of_tables;
        const uint64_t block_headers_mem_size = sizeof(BlockHeader) * number_of_blocks;

        compactionv4::GCompactionMetadata* metadata_host;
        ...
        cudaDeviceSetLimit(cudaLimitDevRuntimePendingLaunchCount, compactionv4::CUDA_QUEUE_LEN);
        assert(number_of_blocks < compactionv4::CUDA_QUEUE_LEN);
        // If there are too many blocks, we cannot compress this file in one go (and some margin)
        if (number_of_blocks > compactionv4::CUDA_QUEUE_LEN - 10) {
            return gtsstErrorTooBig;
        }

        // Some CUDA bookkeeping
        compactionv4::GCompactionMetadata* metadata_gpu;
        ...
        dim3 dimGrid(compactionv4::n_words_per_tile / 32, compactionv4::THREAD_COUNT * number_of_blocks / 32, 1);
        ...
        for (uint32_t i = 0; i < number_of_tables; i++) {
            threads.emplace_back(gpu_create_metadata<symbols::SmallSymbolMatchTableData>, i,
                                 compactionv4::BLOCK_SIZE * compactionv4::SUPER_BLOCK_SIZE, metadata_host,
        ...
        // safeCUDACall(cudaEventRecord(pre_start, compute_stream));
        // compactionv4::transpose_no_bank_conflicts<32, 8><<<dimGrid, dimBlock, 0, compute_stream>>>(
        //     aligned_transpose_dst, aligned_transpose_src);
        // safeCUDACall(cudaEventRecord(pre_done, compute_stream));
        // safeCUDACall(cudaPeekAtLastError());
        ...
        compactionv4::gpu_compaction<<<number_of_blocks, compactionv4::THREAD_COUNT, 0, compute_stream>>>(
            metadata_gpu, block_headers_gpu, dst, tmp + working_block_size, tmp);
