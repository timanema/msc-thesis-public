...
namespace gtsst::compressors::compactionv4 {
    ...
    __global__ void gpu_compaction(GCompactionMetadata* metadata, BlockHeader* headers, const uint8_t* src,
        ...
        assert((uintptr_t)tmp % TMP_WORD_ALIGNMENT == 0); // Ensure 4-byte alignment of dest
        ...
        const auto dst_aligned = (uint32_t*)(tmp + (uint64_t)blockIdx.x * TMP_OUT_BLOCK_SIZE);
        ...
        if (threadIdx.x == 0) {
            const auto aligned_transpose_src = dst_aligned;
            const auto aligned_transpose_dst = (uint32_t*)(dst + (uint64_t)blockIdx.x * TMP_OUT_BLOCK_SIZE);

            dim3 dimGrid(THREAD_COUNT / 32, tile_out_len_words / 32, 1);
