...
namespace gtsst::compressors {
    DecompressionConfiguration CompactionV3Compressor::configure_decompression(size_t buf_size) {
    ...
    GTSSTStatus CompactionV3Compressor::decompress(const uint8_t* src, uint8_t* dst, DecompressionConfiguration& config,
                                                   size_t* out_size) {
        ...
        for (int block_id = 0; block_id < file_header.num_blocks; block_id++) {
            fsst::DecodingTable decoder = decoders[block_id / compactionv3::SUPER_BLOCK_SIZE];
