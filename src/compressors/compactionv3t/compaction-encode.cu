...
namespace gtsst::compressors::compactionv3t {
    __device__ void pack_results_local(uint32_t result[tile_out_word_buf_size][THREAD_COUNT], const uint32_t offset,
                                       const uint32_t val) {
        assert(offset < n_out_symbols_per_chunk);
        ...
        // If the value to be overwritten is not 0xFD (padding) there is an overflow..
        assert((current & ~clean_mask) >> shift == 0xFD);
    ...
    __device__ inline bool can_flush(const uint8_t active_out_block, const uint8_t current_out) {
         ...
         * 1: The current active block is not 0xFDFDFDFD (if flushed, would be in reset state)
    ...
    __device__ void flush(uint8_t* dst, uint32_t result[tile_out_word_buf_size][THREAD_COUNT],
        ...
        // dst_aligned[lane_id * (uint64_t)tile_out_len_words + flushes_before] = word;
        dst_aligned[flushes_before * 32 + lane_id] = word;

        // Reset current block for next use
        result[active_out_block][threadIdx.x] = 0xFDFDFDFD;
    }
    ...
    __device__ EncodeResult compaction_encode_chunked_local(uint8_t* dst, uint32_t flushes_before, uint8_t out,
        ...
        for (int i = 0; i < 8; i++) {
            if (local_in == i && i < spillover_len) {
                ...
                if (escape)
                    pack_results_local(result, (out + 1) % n_out_symbols_per_chunk,
                ...
                out += 1 + escape;
                out %= n_out_symbols_per_chunk; // Make sure that output wraps around
            ...
            if (must_flush(active_out_block, out)) {
                ...
                if (is_early_flush(out, active_out_block)) {
                    ...
                    // Add size of required padding to output size
                    local_out += padding;
    ...
    __global__ void gpu_compaction(GCompactionMetadata* metadata, CompactionV3TBlockHeader<SUB_BLOCKS>* headers,
        ...
        const auto aligned_src = (uint32_t*)(src + BLOCK_SIZE * (uint64_t)blockIdx.x);

        // Make sure result is in the correct state
        for (int i = 0; i < tile_out_word_buf_size; i++) {
            result[i][threadIdx.x] = 0xFDFDFDFD;
        }
        ...
        for (uint32_t chunk_id = 0; chunk_id < n_chunks; chunk_id++) {
            ...
            for (int i = 0; i < tile_word_buf_size; i++) {
                const uint32_t word = aligned_src[(chunk_id * tile_word_buf_size + i) * THREAD_COUNT + threadIdx.x];
                load[i][threadIdx.x] = word;
            }

            // Step 2: Run chunked compression on spillover and loaded window
            const auto encode_result = compaction_encode_chunked_local(
                dst, number_of_words_flushed, block_out_location, active_out_block, m.symbol_table, result, load,
        ...
        while (can_flush(active_out_block, block_out_location)) {
            ...
            if (is_early_flush(block_out_location, active_out_block)) {
                ...
                block_out_location %= n_out_symbols_per_chunk; // Make sure that output wraps around

                // Add size of required padding to output size
                global_size += padding;
            }

            flush(dst, result, number_of_words_flushed, active_out_block);
            active_out_block = (active_out_block + 1) % tile_out_word_buf_size;
        ...
        // Sum block output
        using BlockReduce = cub::BlockReduce<int, THREAD_COUNT>;
        ...
        // Create block header
        if (threadIdx.x == 0) {
            CompactionV3TBlockHeader<SUB_BLOCKS> header = {
                {
