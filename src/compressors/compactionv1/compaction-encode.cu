...
#define USE_SHIFT_ENCODE 0
    ...
    __global__ void gpu_compaction(GCompactionMetadata* metadata, BlockHeader* headers, const uint8_t* src,
        ...
        for (uint32_t chunk_id = 0; chunk_id < n_chunks; chunk_id++) {
            ...
            // for (uint8_t i = fix_block; i < n_stores_per_chunk; i++) {
            //     result[i][threadIdx.x] = 0xFEFEFEFE;
            // }
            ...
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 0] = result[0][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 1] = result[1][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 2] = result[2][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 3] = result[3][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 4] = result[4][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 5] = result[5][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 6] = result[6][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 7] = result[7][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 8] = result[8][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 9] = result[9][threadIdx.x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 10] = result[10][threadIdx.
                x];
            dst_aligned[threadIdx.x * tile_out_len_words + chunk_id * n_stores_per_chunk + 11] = result[11][threadIdx.
                x];
