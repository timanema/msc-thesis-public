...
#include <compressors/compactionv1/compaction-defines.cuh>
#include <compressors/compactionv1/compaction-encode.cuh>
...
namespace gtsst::compressors {
    CompressionConfiguration CompactionV1Compressor::configure_compression(const size_t buf_size) {
        return CompressionConfiguration{.input_buffer_size = buf_size,
                                        .compression_buffer_size = buf_size,
                                        .temp_buffer_size = buf_size * 3,
                                        .min_alignment_input = compactionv1::WORD_ALIGNMENT,
                                        .min_alignment_output = compactionv1::WORD_ALIGNMENT,
                                        .min_alignment_temp = compactionv1::TMP_WORD_ALIGNMENT,
                                        .must_pad_alignment = true,
                                        .block_size = compactionv1::BLOCK_SIZE,
                                        .table_range = compactionv1::BLOCK_SIZE * compactionv1::SUPER_BLOCK_SIZE,
                                        .must_pad_block = true,
    ...
    GTSSTStatus CompactionV1Compressor::validate_compression_buffers(const uint8_t* src, uint8_t* dst, uint8_t* tmp,
                                                                     CompressionConfiguration& config) {
        if (config.input_buffer_size > compactionv1::BLOCK_SIZE * (size_t)0xFFFFFFFF) {
            return gtsstErrorTooBig;
        }

        if (config.block_size != compactionv1::BLOCK_SIZE) {
            return gtsstErrorBadBlockSize;
        }

        if (config.min_alignment_input != compactionv1::WORD_ALIGNMENT ||
            config.min_alignment_output != compactionv1::WORD_ALIGNMENT ||
            config.min_alignment_temp != compactionv1::TMP_WORD_ALIGNMENT) {
            return gtsstErrorBadBlockSize;
        }

        if ((uintptr_t)src % compactionv1::WORD_ALIGNMENT != 0 || (uintptr_t)dst % compactionv1::WORD_ALIGNMENT != 0 ||
            (uintptr_t)tmp % compactionv1::TMP_WORD_ALIGNMENT != 0) {
            return gtsstErrorAlignment;
        }

        if (config.input_buffer_size % compactionv1::BLOCK_SIZE != 0 ||
            config.temp_buffer_size % compactionv1::TMP_OUT_BLOCK_SIZE != 0) {
            return gtsstErrorBlockAlignment;
        }

        if (config.input_buffer_size % compactionv1::WORD_ALIGNMENT != 0 ||
            config.temp_buffer_size % compactionv1::TMP_WORD_ALIGNMENT != 0) {
    ...
    GTSSTStatus CompactionV1Compressor::compress(const uint8_t* src, uint8_t* dst, const uint8_t* sample_src,
        ...
        // Some bookkeeping
        const uint64_t number_of_blocks = config.input_buffer_size / compactionv1::BLOCK_SIZE;
        const uint64_t working_block_size = number_of_blocks * compactionv1::TMP_OUT_BLOCK_SIZE;
        const uint64_t number_of_tables = (number_of_blocks - 1) / compactionv1::SUPER_BLOCK_SIZE + 1;
        const uint64_t metadata_mem_size = sizeof(compactionv1::GCompactionMetadata) * number_of_tables;
        const uint64_t block_headers_mem_size = sizeof(BlockHeader) * number_of_blocks;

        compactionv1::GCompactionMetadata* metadata_host;
        ...
        compactionv1::GCompactionMetadata* metadata_gpu;
        ...
        for (uint32_t i = 0; i < number_of_tables; i++) {
            threads.emplace_back(gpu_create_metadata<symbols::SmallSymbolMatchTableData>, i,
                                 compactionv1::BLOCK_SIZE * compactionv1::SUPER_BLOCK_SIZE, metadata_host,
        ...
        // Run all blocks
        compactionv1::gpu_compaction<<<number_of_blocks, compactionv1::THREAD_COUNT>>>(metadata_gpu, block_headers_gpu,
            src, tmp);
        safeCUDACall(cudaPeekAtLastError());
        safeCUDACall(cudaDeviceSynchronize());
