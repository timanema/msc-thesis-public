...
namespace gtsst::compressors {
    DecompressionConfiguration CompactionV1Compressor::configure_decompression(size_t buf_size) {
    ...
    GTSSTStatus CompactionV1Compressor::decompress(const uint8_t* src, uint8_t* dst, DecompressionConfiguration& config,
                                                   size_t* out_size) {
        ...
        for (int block_id = 0; block_id < file_header.num_blocks; block_id++) {
            fsst::DecodingTable decoder = decoders[block_id / compactionv1::SUPER_BLOCK_SIZE];
